...
public class musicBean extends ParentBean
{
    String id = "-1";
    ...
    {
        return getOnePage("SELECT * FROM music order by addtime desc",cur,records);
    }
    ...
    {
        String sql = " SELECT * FROM music where isuse='1' order by addtime desc";
        return getResultSetData(selectRecord(sql));
    }
    ...
    public int mod(Hashtable hash,String saveDirectory) throws Exception
    {
        ...
        //删除旧的文件
        String file_old = toName("music","id","file",id);
    ...
    public void del(String saveDirectory)
    {
        ...
        deleteRecord("delete from music where id="+id);
    }

    public Vector getFileList(String vid)
    {
        String sql = "select distinct file from music order by addtime";
        if(!id.equals(""))sql = "select distinct file from music where id!="+vid+" order by addtime";
        return getResultSetData(selectRecord(sql));
    }
